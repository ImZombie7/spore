...
static EFI_STATUS open_root(EFI_HANDLE image) {
    ...
    status = bs->handle_protocol(loaded->device_handle,
...
static EFI_STATUS load_modules(struct spore_boot_module *modules, uint32_t *count) {
    ...
    while (next_line(&cursor, line, sizeof(line))) {
        ...
        if (len >= SPORE_BOOT_MODULE_PATH_MAX) {
            return EFI_LOAD_ERROR;
        }
...
static EFI_STATUS load_kernel(struct loaded_file *kernel_file,
    ...
    if (kernel_file->size < sizeof(struct elf64_ehdr)) {
        return EFI_LOAD_ERROR;
    }
    ...
    for (uint16_t i = 0; i < eh->e_phnum; ++i) {
        struct elf64_phdr *ph = (struct elf64_phdr *)((uint8_t *)kernel_file->data + eh->e_phoff +
    ...
    for (uint16_t i = 0; i < eh->e_phnum; ++i) {
        ...
        if (ph->p_offset + ph->p_filesz > kernel_file->size || ph->p_filesz > ph->p_memsz) {
            return EFI_LOAD_ERROR;
        }
...
static void install_ttbr1(uint64_t root_pa) {
    ...
    uint64_t clear = (0x3full << 16) | (1ull << 23) | (3ull << 24) |
                     (3ull << 26) | (3ull << 28) | (3ull << 30);
    tcr &= ~clear;
    tcr |= (16ull << 16) | (1ull << 24) | (1ull << 26) |
           (3ull << 28) | (2ull << 30);
    __asm__ volatile(
        "msr mair_el1, %0\n"
        "msr tcr_el1, %1\n"
...
static int build_page_tables(uint64_t kernel_phys_base,
                             uint64_t kernel_virt_base,
                             uint64_t entry,
                             uint64_t *ttbr1_out) {
    (void)entry;
    ...
    for (uint64_t gb = 0; gb < HHDM_SIZE; gb += 0x40000000ull) {
        ...
        for (uint64_t off = 0; off < 0x40000000ull; off += 0x200000ull) {
            uint64_t pa = gb + off;
            l2[pt_index(HHDM_OFFSET + pa, 21)] =
                (pa & 0x0000ffffffe00000ull) | normal_rw_nx | 0x1ull;
        }
    }
...
static EFI_STATUS final_memory_map(struct spore_memmap_entry *out,
                                   uint32_t *out_count,
                                   EFI_MEMORY_DESCRIPTOR *efi_map,
                                   UINTN efi_map_capacity,
                                   EFI_HANDLE image) {
    for (;;) {
...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *system_table) {
    ...
    if (EFI_ERROR(alloc_pages(pages_for(sizeof(struct spore_boot_module) * MAX_MODULES), (void **)&modules)) ||
        ...
        EFI_ERROR(alloc_pages(16, (void **)&efi_map)) ||
        EFI_ERROR(alloc_pages(1, (void **)&boot)) ||
