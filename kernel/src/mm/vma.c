...
static bool add_raw(struct vma_list *list,
                    uint64_t start,
                    uint64_t end,
                    uint32_t prot,
                    uint32_t flags,
...
static void merge_adjacent(struct vma_list *list) {
    for (size_t i = 0; i < MAX_VMAS; ++i) {
        ...
        for (size_t j = 0; j < MAX_VMAS; ++j) {
            if (i == j || !list->entries[j].used) {
                continue;
            }
            if (list->entries[i].end == list->entries[j].start &&
                compatible(&list->entries[i], &list->entries[j])) {
                list->entries[i].end = list->entries[j].end;
                list->entries[j].used = false;
                j = 0;
            } else if (list->entries[j].end == list->entries[i].start &&
                       compatible(&list->entries[i], &list->entries[j])) {
...
const struct vma *vma_lookup(const struct vma_list *list, uint64_t va) {
    for (size_t i = 0; i < MAX_VMAS; ++i) {
        ...
        if (vma->used && va >= vma->start && va < vma->end) {
            return vma;
        }
    }
    return NULL;
...
bool vma_insert(struct vma_list *list,
                uint64_t start,
                uint64_t end,
                uint32_t prot,
                uint32_t flags,
    ...
    for (size_t i = 0; i < MAX_VMAS; ++i) {
        const struct vma *vma = &list->entries[i];
        if (vma->used && overlaps(start, end, vma->start, vma->end)) {
            return false;
        }
    }
    if (!add_raw(list, start, end, prot, flags, type)) {
        return false;
    }
    merge_adjacent(list);
    return true;
...
bool vma_remove(struct vma_list *list, uint64_t start, uint64_t end) {
    ...
    for (size_t i = 0; i < MAX_VMAS; ++i) {
        ...
        if (vma->start < start &&
            !add_raw(&next, vma->start, start, vma->prot, vma->flags, vma->type)) {
            return false;
        }
        if (end < vma->end &&
            !add_raw(&next, end, vma->end, vma->prot, vma->flags, vma->type)) {
            return false;
        }
    }
    merge_adjacent(&next);
