...
bool elf_load_static_aarch64(struct user_address_space *as,
                             const void *image,
                             size_t image_size,
    ...
    if (kmemcmp(ehdr->e_ident, magic, sizeof(magic)) != 0 ||
    ...
    for (uint16_t i = 0; i < ehdr->e_phnum; ++i) {
        ...
        if (ph->p_memsz < ph->p_filesz ||
            !bounds_ok(ph->p_offset, ph->p_filesz, image_size) ||
        ...
        for (uint64_t va = start; va < end; va += PAGE_SIZE) {
            ...
            if (pa == 0 || !vmm_map_page(as, va, pa, flags | VMM_USER_WRITE)) {
                return false;
            }
